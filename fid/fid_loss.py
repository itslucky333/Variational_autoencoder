...
### This is the new function to handle the imaginary issue
def calculate_frechet_distance(mu1, sigma1, mu2, sigma2, eps=1e-6):
    ...
    -- sigma2: The covariance matrix over activations, precalculated on an
               representative data set.
    -- eps   : A small value to regularize the covariance matrices in case
               of numerical instability (default: 1e-6).
    ...
    # If the square root results in NaN or infinity, apply regularization
    if not np.isfinite(covmean).all():
        msg = "FID calculation produces singular product; adding %s to diagonal of cov estimates" % eps
        print(msg)
        covmean = linalg.sqrtm((sigma1 + np.eye(sigma1.shape[0]) * eps).dot(sigma2 + np.eye(sigma2.shape[0]) * eps))

    # Handle any complex components in the result (numerical artifact)
    if np.iscomplexobj(covmean):
        # Ensure we take only the real part (as imaginary parts are small numerical artifacts)
